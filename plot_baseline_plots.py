...
# Function to parse the baseline log file and extract data
def parse_baseline_log(file_path):
    # Regex pattern to match the epoch summary line
    pattern = r"\* Epoch: \[(\d+)/300\]\s+Top 1-err (\d+\.\d+)\s+Top 5-err (\d+\.\d+)\s+Test Loss (\d+\.\d+)"
...
def plot_error(epochs, top1_err, top5_err, output_path):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Error (%)")
    plt.title("Top-1 and Top-5 Errors")
    plt.legend()
    plt.grid()
    plt.tight_layout()
    plt.savefig(output_path)
...
def plot_accuracy(epochs, top1_accuracy, top5_accuracy, output_path):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy (%)")
    plt.title("Top-1 and Top-5 Accuracies")
    plt.legend()
    plt.grid()
    plt.tight_layout()
    plt.savefig(output_path)
...
if __name__ == "__main__":
    log_file_path = "run_job_randomerase.log"  # Path to the baseline log file

    # Parse the logs and extract data
    epochs, train_loss, top1_err, top5_err = parse_baseline_log(log_file_path)

    # Convert errors to accuracies
    top1_accuracy, top5_accuracy = convert_error_to_accuracy(top1_err, top5_err)

    # Plot and save the images
    plot_loss(epochs, train_loss, "baseline_loss_plot.png")
    plot_error(epochs, top1_err, top5_err, "baseline_error_plot.png")
    plot_accuracy(epochs, top1_accuracy, top5_accuracy, "baseline_accuracy_plot.png")
